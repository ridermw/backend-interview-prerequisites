...
2. Treat 1:1 communication as a channel with 2 members. Talk about tradeoffs.  Analytics where most usage is one or the other.
3. Messages stored chronological order. makes retrieval easier.
4. Users have roles per channel:  Admin, Contributor, Reader
5. use websockets to push messages??
6. use docker to host server.  use Insomnia for REST Client.  Consider separate docker for WS client?
...
2. POST /api/admin/channelMembership.remove -> 200 success; 404 for channel or user if not found.
     { channelId, userId, role}
     // adds user to channel with role.
3. POST /api/channel/message.add -> 200 success; 500 error saving message.
    { channelId, userId, body}
    // posts a message to the channel.
4. GET /api/channeMembership.get -> list of members. 
    // could be queryable to filter list?
4. GET /api/messages.getById(Id) -> return messages since Id.
4. GET /api/messages.getByDateTim(dateTime) -> return messages since dateTime.
